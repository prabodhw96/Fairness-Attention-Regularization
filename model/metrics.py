...
def assess_performance_and_bias(
    ...
    file_directory = output_dir + dataset + "_" + classifier_model

    Path(file_directory).mkdir(parents=True, exist_ok=True)

    # We now save the metrics in a json file
    output_file = file_directory + "/metrics_layer_" + str(layer_id) + ".json"
...
def compute_metrics(
    ...
    with torch.no_grad():
        ...
        for i in range(int(np.ceil(len(split_dataset) / batch_size))):
            ...
            results_after_bias_reduction = model_after_debiasing.forward(
            ...
            results_before_bias_reduction = model_before_debiasing.forward(
                input_ids=torch.tensor(
                    split_dataset.encodings["input_ids"][
                        i * batch_size : (i + 1) * batch_size
                    ]
        ...
        file_path = file_directory + split_name + "_analysis" + "_"  + dataset_name + "_"  + classifier_model  + ".csv"
        #file_directory_exists = False
        #file_directory_exists = exists(file_path)

#        if layer_id != 0 and split_name in ["IPTTS_gender_bias", "IPTTS_social_bias"]:
#            data_IPTTS = pd.read_csv(file_path)
#        else:
#            if split_name == "IPTTS_gender_bias":
#            # If this is the first layer, we load the original csv file. After that, we load the CSV that we edit            
...
#                data_IPTTS = pd.read_csv("./data/" + "bias_madlibs_77k.csv")            
        ...
        if split_name in [
            ...
            if layer_id == 0:
                ...
                # We only save the predictions before debiasing once, to avoid repetition
                data_IPTTS["predcition before debiasing " + "layer " + str(layer_id)] = list(
                    F.softmax(y_pred_before_bias_reduction, 1)[:, 1].cpu().detach().numpy()
                )
            data_IPTTS["prediction after debiasing " + "layer " + str(layer_id)] = list(
                F.softmax(y_pred_after_bias_reduction, 1)[:, 1].cpu().detach().numpy()
            )
            data_IPTTS["Bias contribution for layer " + str(layer_id)] = layer_bias.cpu().detach().numpy()
        ...
        if split_name == "IPTTS_gender_bias":
            ...
            logs["layer id"] = layer_id
            logs["layer bias contribution"] = layer_bias
